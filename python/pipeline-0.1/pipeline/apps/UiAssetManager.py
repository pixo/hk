...
CC_PATH = os.path.join(os.getenv( "HK_PIPELINE"), "pipeline","creative")
...
class UiCreateOnDb(object):
    ...
    def setupUi(self, Form, typ = ( "Type", "typ" ) ):
        #TODO:virer le setup GUI dans un def __init__
        self.typ = typ
        Form.setObjectName("Form")
        Form.resize(486, 429)
...
class UiMainManager(object):
    ...
    def setupUi(self, MainWindow, launcher = "terminal"):
...
#             font.setBold(True)
    ...
    def itemClickedVersion( self, item ):
        ...
        """Set the screenshot"""
        screenshot = "%s_%s.jpg" % ( PROJECT, item.parent().hkid )
...
def sysAM():
    ...
    setupui=UiAssetManager()
    setupui.setupUi( main, "terminal" )
    main.show()
